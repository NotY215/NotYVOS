...
void write(Level lvl, const char* tag, const char* fmt, ...) noexcept
{
    ...
    if (fb::Framebuffer::ready())
    {
        fb::Framebuffer::draw_text(col, g_row, "[", kFg, kBg);
        col += 1;
        fb::Framebuffer::draw_text(col, g_row, level_str(lvl), level_color(lvl), kBg);
        col += 3;
        fb::Framebuffer::draw_text(col, g_row, "] ", kFg, kBg);
        col += 2;
        if (tag)
        {
            fb::Framebuffer::draw_text(col, g_row, tag, 0x0080C0FF, kBg);
            col += 4;
            fb::Framebuffer::draw_text(col, g_row, ": ", kFg, kBg);
            col += 2;
        }
